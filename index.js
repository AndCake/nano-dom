...
HTMLElement.prototype.dispatchEvent = function (event) {
	var sendEvent = Object.assign({}, event, {
	...
	if (this._eventListeners[event.type]) {
		// call them all
		for (var listener, index = 0, len = this._eventListeners[event.type].length; listener = this._eventListeners[event.type][index], index < len; index += 1) {
			var result = listener.call(this, sendEvent);
